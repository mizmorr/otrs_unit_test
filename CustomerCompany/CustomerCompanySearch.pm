...
sub _GetParams {
    ...
    # get single params
    my %GetParam;
    my @SearchFields = $Kernel::OM->Get('Kernel::System::CustomerCompany')->CustomerCompanySearchFields();
    my %LookupSearchFields = map { $_->{Name} => $_ } @SearchFields;
    for my $Item ( sort keys %LookupSearchFields,qw(Limit Result) )
    {
