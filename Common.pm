...
sub ValidateCustomerUser {
    my ( $Self, %Param ) = @_;

    return if !defined $Param{Login} || !length $Param{Login};
    ...
    return if !IsHashRefWithData($CustomerUserData);
...
sub _CheckDynamicField {
	...
	for my $Needed (qw(Name Value)) {
		...
		return {
				ErrorCode    => 'CustomerController.MissingParameter ' . $DynamicField->{Name},
				ErrorMessage => "CustomerController: DynamicField->$Needed parameter $DynamicField->{Value}is missing!",
			};
		}
	        if ( !IsStringWithData( $DynamicField->{$Needed} ) && ref $DynamicField->{$Needed} ne 'ARRAY' ) {
		return {
				ErrorCode    => 'CustomerController.Bad format ',
				ErrorMessage => "CustomerController: DynamicField->$Needed parameter is bad!",
			};
		}
	}
