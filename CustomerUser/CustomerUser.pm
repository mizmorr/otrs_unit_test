...
deletes a customer user from storage

	my $Success = $CustomerUserObject->CustomerUserDelete(
		CustomerUserID	=> 123,
		UserID		=> 123,
);
...
sub CustomerUserDelete {
	...
	# check needed stuff
	for my $Needed (qw(CustomerUserID UserID)) {
		if ( !$Param{$Needed}) {
			$Kernel::OM->Get('Kernel::System::Log')->Log(
				Priority	=> 'error',
				Message		=> "Need $Needed!",
	...
	$Self->CustomerUserCacheClear( CustomerUserID => $Param{CustomerUserID} );
