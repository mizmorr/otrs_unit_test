...
sub Run {
	...
	my %Get = %{ $Self->_GetParams( %{ $SearchDetail } )};
	
	# get dynamic fields
	my %DynamicFieldSearchParameters =  $Self->_GetDynamicFields( %{ $Param{Data}->{SearchDetail}} );

	my @CustomerUserIDs = $Kernel::OM->Get('Kernel::System::CustomerUser')->CustomerSearchDetail(
		%Get,
...
sub _GetParams {
	...
	my %LookupCustomerUserSearchFields = map { $_->{Name} => $_ } @AllSearchFields;

	for my $Item ( sort keys %LookupCustomerUserSearchFields, qw(Limit Result)){
...
get search parameters.
    ...
    my %DynamicFieldSearchParameters = _GetDynamicFields(
        %Params,                          # all customer user parameters
    );
