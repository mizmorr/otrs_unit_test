...
sub _CustomerUserAdd {
 ...
 #   my $IsExist = $CustomerUserObject->CustomerSearch(
...
#    ) if ($IsExist);
	...
	return $Self->ReturnError(
		...
		ErrorMessage => "CustomerUserCreate: Email address (" . $CustomerUser->{UserEmail} . ") not valid (". $CheckItemObject->CheckError() . ")!",
	) if !$ValidateEmail;
...
#TODO: need to think about purposes of validation

	#return $Self->ReturnError(
	#	ErrorCode => 'CustomerUserCreate.ValidateCustomery',
	#	ErrorMessage =>"CustomerUserCreate: UserCustomerID does not exist!",
	#) if (defined $CustomerUser->{UserCustomerID} && !$Self->ValidateCustomerUser( Login => $CustomerUser->{UserCustomerID} ));
